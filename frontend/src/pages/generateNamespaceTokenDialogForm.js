...
const GenerateNamespaceTokenDialogForm = ({ namespace, show, onHide }) => {
  ...
  const { successMessage, errorMessage, uploadToken, isLoading } = useSelector(
    (state) => state.generateNamespaceToken
  );
  ...
  return (
    <Modal
      ...
      onHide={onHide}
      size="md"
      aria-labelledby="generate-namespace-token-modal"
      centered
      onExited={resetData}
    >
      <Modal.Header closeButton>
        <Modal.Title id="generate-namespace-token-modal">
          Generate Namespace Token
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <p className="mb-3">
          Generate an upload token for namespace <strong>{namespace}</strong>.
        </p>
        <p className="text-muted small mb-3">
          This token can be used to upload packages to this namespace via the CLI.
        </p>
